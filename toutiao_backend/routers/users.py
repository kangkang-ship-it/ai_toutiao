...
router = APIRouter(prefix="/api/user", tags=["users"])  # 把 users 改成 user

@router.post("/register")
async def register(user_date: UserRequest,db: AsyncSession = Depends(get_db)):  # 用户信息 和 db
    # 注册逻辑：验证用户是否存在 -> 创建用户 → 生成 Token  → 响应结果
    existing_user = await users.get_user_by_username(db, user_date.username)
    if existing_user:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="用户已存在")
    user = await users.create_user(db, user_date)
...
# 查Token查用户 -> 封装crud -> 功能整合一个工具函数 ->路由导入:依赖注入
@router.get("/info")
async def get_user_info(user:User = Depends(get_current_user)):
    return success_response(message="获取用户信息成功",data=UserInfoResponse.model_validate(user))

#修改用户信息: 验证Token->更新(用户输入数据,put提交->请求体参数->定义Pydantic模型类)->响应结果
#参数:用户输入 + 验证Token的 + db(代用更新的方法)
@router.put("/update")
async def update_user_info(user_date: UserUpdateRequest,user: User = Depends(get_current_user),db: AsyncSession = Depends(get_db)):
    user = await users.update_user(db, user.username, user_date)
